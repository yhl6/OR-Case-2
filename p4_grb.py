...
def heuristic_algorithm(num):
    ...
    # 其他資料
    Vendor_product = pd.read_excel(xls, sheet_name='Vendor-Product', index_col=0, dtype=np.int64)
    ...
    # Demand.values.sum() = Demand.values.sum()
    ...
    for t in MonthID:
        for k in Shipping_method:
            p2.addConstr(quicksum(x[i, k, t] for i in ProductID) <= Demand.values.sum() * z[k, t], 'Z constraint')

    # Y的binary constraint
    for i in ProductID:
        for t in MonthID:
            p2.addConstr(Demand.values.sum() * d[i, t] >= s[i, t])
            p2.addConstr(Demand.values.sum() * (1 - d[i, t]) >= v[i, t])
    ...
    for r in VendorID:
        for t in MonthID:
            for i in [ProductID_grouped.groups[r]]:
                p2.addConstr((quicksum(x.select(i, '*', t))) <= Demand.values.sum() * o[r, t])

    # Lower Bound constraint
    p2.addConstrs((quicksum(x.select(i, '*', t)) >= Lb[i] * a[i, t]) for i in ProductID for t in MonthID)
    p2.addConstrs((quicksum(x.select(i, '*', t)) <= Demand.values.sum() * a[i, t]) for i in ProductID for t in MonthID)
    ...
    # print(f's{num}\'s gap: {100 * (p2.objVal - optimal_sol[num - 1]) / optimal_sol[num - 1]}%')
    print('-------------')
