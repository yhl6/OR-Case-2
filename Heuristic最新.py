...
def heuristic_algorithm(file_path,n):
	...
	z = (sum(Express_delivery[i]*x[i][1] for i in ProductID) 
	 + sum(Air_freight[i]*x[i][t] for t in range(2,26) for i in Air)
	 + 1500 * (sum(math.ceil((x[i][t]*Cubic_meter[i])/0.5) for i in Ship for t in range(2,26)))
	 + sum(Purchasing_cost[i]*x[i][t] for t in MonthID for i in ProductID)
	 + sum(Holding_cost[i]*y[i][t] for t in MonthID for i in ProductID)
	 + sum(Lost_sales[i]*(1-Backorder_percentage[i])*x[i][0] for i in ProductID)
	 + Fixed_cost[0] + Fixed_cost[1]*25
	 + (Vendor_cost[0]+Vendor_cost[1])*26 )
